 ...
 * <p/>
 * The preferred size of the list is determined by the text bounds of the items and the size of the {@link Style#selection}.
 */
@SuppressWarnings("unused")
public class KrList<T> extends Widget implements Cullable {
    private Style style;
    ...
    public KrList(Skin skin) {
        this(skin.get("default", Style.class));
    }

    public KrList(Style style) {
    ...
    /**
     * Returns the list's style. Modifying the returned style may not have an effect until {@link #setStyle(Style)} is called.
     */
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        for (int i = 0; i < items.size; i++) {
            if (cullingArea == null || (itemY - itemHeight <= cullingArea.y + cullingArea.height && itemY >= cullingArea.y)) {
                ...
                font.draw(batch, item.toString(), x + textOffsetX + 4, y + itemY - textOffsetY);
                if (selected) {
    ...
    static public class Style {
        ...
        @Getter
        private final Color fontColorSelected = new Color(1, 1, 1, 1);

        @Getter
        private final Color fontColorUnselected = new Color(1, 1, 1, 1);
